# claude-team-usage 再開手順

このドキュメントは、Claude Code セッションを終了したり別の Mac に切り替えた後で、
`claude-team-usage` の開発・運用作業をスムーズに再開するためのチェックリスト。
...
| 項目 | 値 |
|---|---|
| 公開 URL | `https://app.instyle.group/claude-team-usage/` |
| GitHub | `https://github.com/sasaki-ta-instyle/claude-team-usage`（Private 想定） |
| ConoHa デプロイ先 | `/var/www/app/claude-team-usage/` |
| 共有 env | `/var/www/_shared/apps/app-claude-team-usage.env`（chmod 600） |
| PM2 名 | `app-claude-team-usage` |
| ポート | `3009` |
| Healthcheck | `/claude-team-usage/api/health` |
| USE_DB | `true` |
...
```bash
mkdir -p ~/Workspace
gh repo clone sasaki-ta-instyle/claude-team-usage ~/Workspace/claude-team-usage
cd ~/Workspace/claude-team-usage
```
...
`~/Workspace/claude-team-usage/.env.local` に必要なキーを揃える（**git に入れない**）。
本番 `app-claude-team-usage.env` と概ね同じ値で動く。
...
pnpm dev
# → http://localhost:3009/claude-team-usage/ にアクセス
```
...
| 種類 | 場所 | 引き継ぎ方法 |
|---|---|---|
| ソースコード | GitHub `sasaki-ta-instyle/claude-team-usage` | `git clone` |
| Claude Code 設定（memory / agents / skills） | `instyle-claude-sasaki` リポジトリ | bootstrap で同期 |
| 本番 Web プロセス（PM2） | ConoHa `app-claude-team-usage` | 触らない、`deploy-prod.yml` で更新 |
| 本番 env（API キー類） | ConoHa `/var/www/_shared/apps/app-claude-team-usage.env` | サーバ側永続、`ssh conoha-deploy` で参照可 |
| 本番 DB（SQLite の場合） | ConoHa `/var/www/app/claude-team-usage/data/...` | サーバ側永続 |
...
```bash
gh workflow run deploy-prod.yml --ref main -R sasaki-ta-instyle/claude-team-usage
gh run watch -R sasaki-ta-instyle/claude-team-usage
```
...
```bash
ssh conoha-deploy '
sed -i "s|^KEY_NAME=.*|KEY_NAME=new_value|" /var/www/_shared/apps/app-claude-team-usage.env
cd /var/www/app/claude-team-usage/current && pm2 startOrReload ecosystem.config.cjs --update-env
'
```
...
```bash
ssh conoha-deploy 'pm2 logs app-claude-team-usage --nostream --lines 50 --raw'
```

### 本番 PM2 再起動

```bash
ssh conoha-deploy 'pm2 restart app-claude-team-usage --update-env'
```
