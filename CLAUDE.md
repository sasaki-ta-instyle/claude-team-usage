...
| `/` | KPI 7 枚（コスト内訳 / アクティブ数 / トークン / Premium 推奨 / Standard 維持 / API 直渡し候補 / 未使用） | **OTel push 統合**（Cowork + Code）＋ `users` roster |
| `/members?days=N` | メンバー別テーブル（Cowork / Code / 合計 / 稼働日 / 最大日コスト / api_error / 推奨 seat 4 値）。**合計コスト一本で判定**（≥ $100 = Premium / $10–$100 = Standard / < $10 = API 直渡し / プロンプト 0 = 未使用）。副バッジで `cap 到達`（api_error 観測）/ `高稼働`（≥ 15 日）を表示 | **OTel push 統合**＋稼働シグナル |
...
| `/simulate` | Premium / Standard 配分シミュレーション（**既定閾値 $100 = 損益分岐**、API 直渡し候補 → 集計外 トグル付き） | **OTel push 統合**＋ `users` roster |
