...
| `/members?days=N` | メンバー別テーブル（Cowork / Code / 合計 / 稼働日 / 最大日コスト / api_error / 推奨 seat 4 値）。**合計コスト一本で判定**（≥ $100 = Premium / $10–$100 = Standard / < $10 = API 従量 / プロンプト 0 = 未使用）。副バッジで `cap 到達`（api_error 観測）/ `高稼働`（≥ 15 日）を表示 | **OTel push 統合**＋稼働シグナル |
