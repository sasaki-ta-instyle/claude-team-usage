...
| `REPORT_HTML_DEST` | scp 転送先 (`<host>:<dir>`) | `conoha-root:/var/www/app/html` |
...
```
0 3 1 * * cd /var/www/app/claude-team-usage/current && set -a && . /var/www/_shared/apps/app-claude-team-usage.env && set +a && node scripts/monthly-report.mjs >> /var/log/app-claude-team-usage-monthly.log 2>&1
```

### 初回反映手順（手動）

1. main を push（この節を含む変更）→ `gh workflow run deploy-prod.yml --ref main && gh run watch`
2. サーバから 1 回だけ手動で前月分を生成:
   ```bash
   ssh conoha-deploy 'cd /var/www/app/claude-team-usage/current \
     && set -a && . /var/www/_shared/apps/app-claude-team-usage.env && set +a \
     && node scripts/monthly-report.mjs'
   ```
3. `curl -sSLo /dev/null -w "%{http_code}\n" https://app.instyle.group/html/claude-team-usage-<前月>.html` で 200 確認
4. `bash ~/Workspace/workspace-index/deploy.sh` で索引ページを反映
5. crontab に月次エントリを追加
