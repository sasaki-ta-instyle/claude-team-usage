...
| `/` | KPI 4 枚（コスト内訳 / アクティブ数 / トークン / Premium 候補数） | **OTel push 統合**（Cowork + Code） |
| `/members?days=N` | メンバー別テーブル（Cowork コスト / Code コスト / 合計 / Premium 候補） | **OTel push 統合** |
...
| `/api-messages?days=N` | API Messages 集計（account×workspace×api_key×model 単位） | Anthropic Admin API |
| `/simulate` | Premium / Standard 配分シミュレーション（閾値 $50） | **OTel push 統合** |
...
```
スクリプトは `~/.zshrc` または `~/.bashrc` に env を冪等追記。手順は `instyle-claude-team` の `install.html` の「Claude Team 使用量の可視化」セクションも同期して更新する。
